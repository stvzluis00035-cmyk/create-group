...
async def initialize_user_client(user_id):
    ...
    try:
        ...
        try:
            ...
            print(f"✅ Usuario {user_id} - {username} conectado")
        except Exception as e:
            print(f"⚠️ Error obteniendo info del usuario {user_id}: {e}")
            user_sessions[user_id]["username"] = "Error obteniendo username"
            
        return True
    except Exception as e:
        print(f"Error inicializando cliente de usuario {user_id}: {e}")
        user_sessions[user_id]["active"] = False
        user_sessions[user_id]["username"] = "Error de conexión"
        return False
...
async def enviar_mensajes_aleatorios(client, group_id, language, username):
    """Envía 8 mensajes con palabras aleatorias al grupo, incluyendo el username"""
    try:
        ...
        for i in range(8):
            ...
            # Agregar el username al mensaje (cada 2-3 mensajes)
...
async def crear_grupos(message: Message, user_id, language):
    ...
    try:
        ...
        progress_msg = await message.reply(f"🔄 Creando 50 grupos en {language} con sesión {username}...\n\n0/50 completados")
        ...
        for i in range(1, 51):
            try:
                ...
                group = await client.create_supergroup(
                    title=group_name,
                    description=f"Grupo automático #{i} - Sesión: {username}"
                )

                # Obtener ID del grupo
                group_id = group.id
                
                # Enviar mensajes aleatorios al grupo (incluyendo username)
                mensajes_enviados = await enviar_mensajes_aleatorios(client, group_id, language, username)
                
                grupos_creados += 1
                resultado = f"✅ Grupo {i}: {group_name} (Sesión: {username})"
                ...
                if i % 5 == 0:
                    await progress_msg.edit_text(
                        f"🔄 Creando grupos en {language} con sesión {username}...\n\n{i}/50 completados\n"
                ...
                resultados.append(f"❌ Grupo {i}: {error_msg} (Sesión: {username})")
...
def register_handlers():
    ...
    async def handle_callbacks(client: Client, callback_query):
        ...
        if data.startswith("confirm_yes_"):
            ...
            username = user_sessions.get(user_id, {}).get("username", "Usuario")
            ...
            await callback_query.message.edit(
                ...
                f"👤 Sesión que se usará: {username}\n\n"
                "Esto puede:\n"
            ...
            client = user_data["client"]
            username = user_data.get("username", "Usuario")
            await callback_query.message.edit(f"🧪 Probando con 2 grupos en {selected_language} (Sesión: {username})...")
            ...
            for i in range(1, 3):
                try:
                    ...
                    group = await client.create_supergroup(
                        title=group_name,
                        description=f"Grupo prueba #{i} - Sesión: {username}"
                    )
                    
                    # Enviar mensajes aleatorios al grupo de prueba (incluyendo username)
                    await enviar_mensajes_aleatorios(client, group.id, selected_language, username)
                    
                    grupos_test_creados += 1
                    await callback_query.message.reply(f"✅ Grupo {i}: {group_name} (Sesión: {username})")
                    await asyncio.sleep(3)
                except Exception as e:
                    error_msg = "Límite de flood" if "FLOOD" in str(e).upper() else str(e)
                    await callback_query.message.reply(f"❌ Error {i}: {error_msg} (Sesión: {username})")
                    break
            
            await callback_query.message.reply(f"🧪 Prueba completada: {grupos_test_creados}/2 grupos (Sesión: {username})")
    ...
    async def estado_command(client: Client, message: Message):
        ...
        # Obtener información del username si existe la sesión
        username_info = ""
        if user_has_session:
            username = user_sessions[user_id].get("username", "Sin username")
            username_info = f"👤 Usuario de sesión: {username}\n"
            ...
            try:
                if state["step"] == "phone":
                    ...
                    try:
                        ...
                        # Obtener información del usuario (username)
