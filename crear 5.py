...
WORDS = {
    ...
    "árabe": [
        ...
        "بحر", "جبل", "شاطئ", "سماء", "سحابة", "نجمة", "وقт", "ساعة",
...
async def enviar_mensajes_aleatorios(client, group_id, language):
    """Envía 8 mensajes con palabras aleatorias al grupo"""
    try:
...
async def crear_grupos(message: Message, user_id, language):
    ...
    try:
        ...
        progress_msg = await message.reply(f"🔄 Creando 50 grupos en {language}...\n\n0/50 completados")
        ...
        for i in range(1, 51):
            try:
                ...
                # Enviar mensajes aleatorios al grupo
...
def register_handlers():
    ...
    async def crear_grupos_command(client: Client, message: Message):
        ...
        try:
            ...
            await message.reply(
                "🌍 **Selecciona el idioma para los mensajes aleatorios:**",
    ...
    async def handle_callbacks(client: Client, callback_query):
        ...
        if data.startswith("confirm_yes_"):
            ...
            await callback_query.message.edit(
                f"⚠️ **ADVERTENCIA** ⚠️\n\n"
                f"Vas a crear 50 grupos con mensajes aleatorios en {selected_language}. Esto puede:\n"
            ...
            client = user_data["client"]
            await callback_query.message.edit(f"🧪 Probando con 2 grupos en {selected_language}...")
            ...
            for i in range(1, 3):
                try:
                    ...
                    # Enviar mensajes aleatorios al grupo de prueba
                    await enviar_mensajes_aleatorios(client, group.id, selected_language)
                    
                    grupos_test_creados += 1
                    await callback_query.message.reply(f"✅ Grupo {i}: {group_name}")
                    await asyncio.sleep(3)
                except Exception as e:
                    error_msg = "Límite de flood" if "FLOOD" in str(e).upper() else str(e)
                    await callback_query.message.reply(f"❌ Error {i}: {error_msg}")
                    break
            
            await callback_query.message.reply(f"🧪 Prueba completada: {grupos_test_creados}/2 grupos")
        
        await callback_query.answer()

  #  @bot_client.on_message(filters.comma
    ...
    async def test_command(client: Client, message: Message):
        ...
        try:
            ...
            await message.reply(
                "🌍 **Selecciona el idioma para la prueba:**",
                reply_markup=keyboard
            )
...
async def main():
    ...
    # Inicializar cliente del bot
    if not initialize_bot_client():
        print("❌ Error inicializando bot client")
        return
    ...
    try:
        ...
        # Inicializar clientes de usuarios existentes
        ...
        # Guardar sesiones y detener clientes
        save_sessions()
        
        # Detener cliente del bot
        if bot_client and bot_client.is_connected:
            await bot_client.stop()
        
        # Detener clientes de usuarios
